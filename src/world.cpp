...
struct BlockTypeMetadata {
    std::string name;
    Texture2D tex;
};

BlockTypeMetadata BlockTypeMetadataRegistry[static_cast<u64>(BlockType::BLOCKCOUNT)];

void registerBlock(BlockType t, std::string name) {
    u64 typeIndex = static_cast<u64>(t);
    BlockTypeMetadataRegistry[typeIndex].name = name;

    Texture2D tex = LoadTextureSafe(TextFormat("assets/textures/%s.png", name.data()));
    BlockTypeMetadataRegistry[typeIndex].tex = tex;
}
...
constexpr const BlockTypeMetadata& GetBlockData(BlockType t) {
    return BlockTypeMetadataRegistry[static_cast<u64>(t)];
}

constexpr const Texture2D& GetBlockTexture(BlockType t) {
    return BlockTypeMetadataRegistry[static_cast<u64>(t)].tex;
}
...
void DrawWorldInCamera(World &world, Camera2D& camera) {	
	...
	i64 blocksToLeft = (screen_width / 2) + 1;
	i64 blocksToRight = (screen_width / 2) + 1;
	i64 blocksToUp = (screen_height / 2) + 1;
	i64 blocksToDown = (screen_height / 2) + 1;
...
void WorldBorderBodyCollide(World &world, Body &body, double dt) {
	const float mapWidthFloat = static_cast<float>(world.realWidth);
	const float mapHeightFloat = static_cast<float>(world.realHeight);

	if (body.position.x + body.velocity.x * dt < 0.0f) {
		body.position.x = 0.0f;
		body.velocity.x = 0.0f;
	}
	else if (body.position.x + body.size.x + body.velocity.x * dt > mapWidthFloat) {
		body.position.x = mapWidthFloat - body.size.x;
		body.velocity.x = 0.0f;
	}

	if (body.position.y + body.velocity.y * dt < 0.0f) {
		body.position.y = 0.0f;
		body.velocity.y = 0.0f;
	}
	else if (body.position.y + body.size.y + body.velocity.y * dt > mapHeightFloat) {
		body.position.y = mapHeightFloat - body.size.y;
		body.velocity.y = 0.0f;
	}
}
...
World GenerateWorld(u64 width, u64 height, u64 seed) {
	...
	world.width = width;
	world.height = height;
	world.realHeight = height * BLOCK_SIZE;
	world.realWidth = width * BLOCK_SIZE;

	world.data.assign(world.height, std::vector(world.width, BlockData{BlockType::AIR}));
	...
	i64 midHeight = world.height / 2;
	for (i64 x = 0; x < world.width; x++) {
		
		float noiseValue = noise.GetNoise(static_cast<float>(x), 0.0f);
		i64 y0 = midHeight + static_cast<i64>(noiseValue * 6);

		for (i64 y = world.height - 1; y >= y0; y--) {
			world.data[y][x] = BlockData{BlockType::DIRT};
		}
	}

	std::cout << "\n";

	std::cout << "Done returning the world" << std::endl;
	std::cout << world.width << " " << world.height << " " << std::endl;
